              ...
              <tr key={journal.id} className="hover:bg-gray-50 transition-colors">
                <td className="px-4 py-4">
                  <Checkbox 
                    ...
                    onCheckedChange={(checked) => handleSelectJournal(journal.id, checked)}
                  />
                </td>
                <td className="px-4 py-4">
                  <Link href={`/journals/${journal.slug}`} className="text-sm font-medium text-gray-900 hover:text-blue-600">
                    {journal.name}
                  </Link>
                </td>
                ...
                <td className="px-4 py-4">
                  {journal.category && journal.category.trim() !== '' ? (
                    <Badge 
                      variant="outline" 
                      className={`text-xs ${getCategoryBadgeColor(journal.category)}`}
                    >
                      {journal.category}
                    </Badge>
                  ) : (
                ...
                <td className="px-4 py-4">
                  <Link href={`/journals/${journal.slug}`} className="text-gray-400 hover:text-blue-600">
